...
export default function Sidebar({ onOpenBgSelector }) {
  ...
  return (
    <div
      className={`drop-shadow-md p-3 h-screen bg-amber-100/50 dark:bg-gray-600/50 flex flex-col transition-all duration-300 ${
        isCollapsed ? "w-20" : "w-1/6"
      }`}
    >
      <div className="flex items-center justify-between gap-2 mb-4">
        {!isCollapsed && (
          <h1 className="text-center text-3xl dark:text-gray-50 flex-1">
            Calendeer
          </h1>
        )}
        <button
      ...
      <div className="flex-1">
        <TabSidebar TabItems={TabItems} isCollapsed={isCollapsed} />
      </div>
    </div>
  );
}
